...
/* ---------- constants ---------- */
// ... (keep existing constants: WIDTH, HEIGHT, NODE_RADIUS, ARROW_SIZE, PADDING)
const PADDING = 50;
const WIDTH = 960;
const HEIGHT = 800;
...
export default function CourseGraph() {
  ...
  const nodeColor = useCallback((id) => {
    // ... (keep existing nodeColor logic - no changes needed here)
    if (mode === 'completed') {
  ...
  // Keep setupSvg, setupDefs, setupZoom, setupLayers as they are
  ...
  const setupDefs = (svgSelection) => {
      ...
      defs.append('marker')
          ...
          .append('path')
          .attr('d', `M0,${-ARROW_SIZE / 2} L${ARROW_SIZE},0 L0,${ARROW_SIZE / 2} Z`)
          .attr('fill', '#999');
  };
  ...
  const updateGraphVisuals = useCallback((nodeSelection, colorLogic, clickHandler) => {
   ...
   const handleNodeClick = useCallback((id) => {
    ...
    }, [mode, toggleCompleted]); // Depends on mode and toggleCompleted callback
